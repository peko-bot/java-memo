...
import com.orz.po.HeaderChunks;
...
public class ReadMidi {
	...
	public void readDateFIle(File midiFile) throws Exception {
		...
		List<TrackChunk> trackChunklist = this.readTreackChunks(byteBuffer, headerChunks);
		this.outInforMidi(headerChunks, trackChunklist);
	}

	public void outInforMidi(HeaderChunks headerChunks, List<TrackChunk> trackChunklist) {
		...
		for (int i = 0; i < trackChunklist.size(); i++) {
			...
			System.out.println("音轨块" + i + ":" + new String(trackChunklist.get(i).getMidiId()));
	 ...
	 * @return
	 */
	public List<TrackChunk> readTreackChunks(ByteBuffer buffer, HeaderChunks headerChunks) {
